...
describe('Starting tests for cc1net', function() {
  ...
  it('website title should be cc1net', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('cc1net');
    })
  });

  it('network-name should be cc1net@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('cc1net@0.0.1.bna');
    });
  });

  it('navbar-brand should be cc1net',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('cc1net');
    });
  });
